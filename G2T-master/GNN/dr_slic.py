...
class SLIC(object):
    ...
    # def get_Cos_distance(self):
    #     min = -1
    #     max = 1
    #     S = torch.from_numpy(self.S)
    #     St = torch.t(S)
    #     # todo tips 2: 余弦相似度衡量两个超像素样本之间的相似度
    #     keep_pixel_ave = torch.mean(S, dim=-1, keepdim=True).expand_as(S)
    #     keep_pixel_t_ave = torch.mean(St, dim=-2, keepdim=True).expand_as(St)
    #     S = S - keep_pixel_ave
    #     St = St - keep_pixel_t_ave
    #     # 上面的数据是准备用来做修正后的余弦相似度的,若使用普通余弦相似度，注释即可
    #     dot_matrix = S @ St
    #     row_norm = torch.norm(S, p=2, dim=-1, keepdim=True)
    #     col_norm = torch.norm(St, p=2, dim=-2, keepdim=True)
    ...
    #     Cos_distance = torch.mean(matrix, dim=-2, keepdim=True).expand_as(matrix)
    #     Cos_distance = Cos_distance.numpy()
    #     return Cos_distance
...
class LDA_SLIC(object):
    ...
    def LDA_Process(self, curr_labels):
        ...
        x=self.x_flatt[idx]
        y=curr_labels[idx]
        pca = LinearDiscriminantAnalysis() # n_components = self.n_component
        pca.fit(x,y-1)
...
#         height, width, bands = HSI.shape  # 512*217*15 # 原始高光谱数据的三个维度
#         data = np.reshape(HSI, [height * width, bands])  # 111104*15
#         minMax = preprocessing.StandardScaler()
#         data = minMax.fit_transform(data)
#         self.data = np.reshape(data, [height, width, bands])
#         self.labels = labels
...
#
#         for i in range(superpixel_count):
#             idx = np.where(segments == i)[0]  # 常用该形式获取数组中满足条件的元素位置
#             count = len(idx)
#             pixels = x[idx]
#             superpixel = np.sum(pixels, 0) / count  # sum()输入参数带有axis时，将按照指定axis进行对应求和
#             S[i] = superpixel
#             Q[idx, i] = 1
#
#         self.S = S
...
#         lda.fit(x, y - 1)
#         X_new = lda.transform(self.x_flatt)  # 111104*15
#         return np.reshape(X_new, [self.height, self.width, -1])  # 512*217*15
#
#     def SLIC_Process(self, img, scale=25):
#         n_segments_init = self.height * self.width / scale  # 1111
...
#     def simple_superpixel(self, scale):
#         curr_labels = self.init_labels
#         X = self.LDA_Process(curr_labels)  # 512*217*15
#         Q, S, A, Seg = self.SLIC_Process(X, scale=scale)
#         return Q, S, A, Seg
#
#     def simple_superpixel_no_LDA(self, scale):
